package handler

import (
	"log"
	"memrizr/account/model"
	"memrizr/account/model/apperrors"
	"net/http"

	"github.com/gin-gonic/gin"
)

// Me handler calls services for getting a user's details
func (h *Handler) Me(c *gin.Context) {
	// c.JSON(http.StatusOK, gin.H{
	// 	"hello": "it's me",
	// })

	// A *model.User will eventually be added to context in middleware
	user, exist := c.Get("user")

	// This shouldn't happer, as our middleware ought to throw an error.
	// This is an extra safety measure
	// We'll extract this logic later as it will common to all handler
	// methods which require a valid user
	if !exist {
		log.Print("Unable to extract user from request context for unknown reason")
		err := apperrors.NewInternal()
		c.JSON(err.Status(), gin.H{
			"error": err,
		})

		return
	}

	uid := user.(*model.User).UID

	// gin Context satisfies go's context.Context interface
	u, err := h.UserService.Get(c, uid)

	if err != nil {
		log.Printf("Unable to find user: %v\n%v", uid, err)
		e := apperrors.NewNotFound("user", uid.String())

		c.JSON(e.Status(), gin.H{
			"error": e,
		})

		return
	}

	c.JSON(http.StatusOK, gin.H{
		"user": u,
	})
}